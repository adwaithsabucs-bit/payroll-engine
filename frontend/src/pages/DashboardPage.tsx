  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setStats({ labourers, contractors, attendance, pending_payroll: pending, approved_payroll: approved, total_wages: totalWages });
      } catch (err) {
        ...
        <div className="dash-header">
          ...
          <div style={{ textAlign:'right' }}>
            ...
            <div className="dash-date">
              {time.toLocaleDateString('en-IN', { weekday:'long', day:'numeric', month:'long', year:'numeric' }).toUpperCase()}
            </div>
          </div>
        </div>
          ...
          <>
            <div className="dash-grid">
              {cards.map(card => {
                ...
                return (
                  <div key={card.label} className="dash-stat" style={{ '--stat-color': card.color } as any}>
            ...
            <div className="dash-info-grid">
              <div className="dash-info-card">
                ...
                <div className="dash-info-row"><span>Logged in as</span><strong>{user?.username}</strong></div>
                <div className="dash-info-row"><span>Access level</span><span className="dash-role-badge">{user?.role}</span></div>
                <div className="dash-info-row"><span>Email</span><strong>{user?.email || '—'}</strong></div>
              </div>
              <div className="dash-info-card">
