...
import { CheckCircle, XCircle, Clock, Eye, Plus, X, UserPlus } from 'lucide-react';
...
const pageStyle = `@keyframes pageIn{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}`;
...
// FIX: was two parallel .then() chains causing a race condition where
// the contractor init would overwrite the pre-filled attendance records.
// Now uses a single Promise.all → merge before any setState calls.
function SupervisorAttendanceView() {
  ...
  // Load contractors + existing attendance atomically (no race condition)
  useEffect(() => {
    if (!selectedProject) return;
    setLoadingData(true);

    const contractorUrl = `/workforce/contractors/?supervisor_project=${selectedProject}`;
    const attendanceUrl = `/attendance/contractor-attendance/?project=${selectedProject}&date=${date}`;

    Promise.all([
      apiClient.get(contractorUrl).catch(() =>
        apiClient.get('/workforce/contractors/')
      ),
      apiClient.get(attendanceUrl).catch(() => ({ data: [] })),
    ]).then(([cRes, aRes]) => {
      const list    = extractResults<any>(cRes.data);
      const records = extractResults<any>(aRes.data);

      // Build attendance map: default PRESENT for all, then override with existing
      const init: Record<number, string> = {};
      list.forEach((c: any) => { init[c.id] = 'PRESENT'; });
      records.forEach((rec: any) => {
        // rec.contractor is the Contractor model's PK (not CustomUser PK)
        if (rec.contractor !== undefined) {
          init[rec.contractor] = rec.status;
        }
      });

      setContractors(list);
      setAttendance(init);       // single atomic update — no race condition
    }).catch(e => console.error(e))
  ...
  return (
    <div style={{ animation: 'pageIn 0.4s ease' }}>
      ...
      {loadingData ? (
        <div style={{ ...S.card, color: '#52525b', textAlign: 'center', padding: 40 }}>Loading contractors...</div>
      ) : contractors.length === 0 ? (
        <div style={{ ...S.card, color: '#3f3f46', textAlign: 'center', padding: 60 }}>No contractors assigned to this project.</div>
      ) : (
...
function ContractorAttendanceView() {
  ...
  const loadLabourers = useCallback(async () => {
    ...
    try {
      ...
      atts.forEach((a: any)  => {
        ...
        init[key] = { status: a.status, ot: String(a.overtime_hours || 0) };
      });
  ...
  const handleAddTemp = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post('/attendance/temp-labourers/', {
        ...tempForm, contractor: myContractorId,
        daily_wage: parseFloat(tempForm.daily_wage),
      });
      setShowTempForm(false);
      setTempForm({ name: '', phone: '', skill: '', daily_wage: '0' });
      loadLabourers();
    } catch { alert('Failed to add temp labourer.'); }
  };
  ...
  const AttRow = ({ label, keyPrefix, id, isTemp }: any) => (
    <div style={{ display: 'grid', gridTemplateColumns: '40px 1.5fr repeat(4,1fr) 80px', padding: '13px 16px', background: '#0d0d0d', borderBottom: '1px solid #111', alignItems: 'center', gap: 8 }}>
      <span style={{ fontSize: 10, color: isTemp ? '#facc15' : '#52525b', fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 700 }}>{isTemp ? 'T' : 'F'}</span>
      <div>
      ...
      <input type="number" min="0" step="0.5" placeholder="OT"
  ...
  return (
    <div style={{ animation: 'pageIn 0.4s ease' }}>
      ...
      {msg && (
        <div style={{ background: msg.includes('saved') ? 'rgba(22,163,74,0.07)' : 'rgba(220,38,38,0.07)', borderLeft: `3px solid ${msg.includes('saved') ? '#16a34a' : '#dc2626'}`, padding: '10px 16px', marginBottom: 20, fontSize: 13, color: msg.includes('saved') ? '#4ade80' : '#fca5a5', borderRadius: 4 }}>
          {msg}
        </div>
      )}
