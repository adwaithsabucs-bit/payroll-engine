...
const CreateFormFields = ({ f, setF, contractors, supervisors }: CreateFieldsProps) => (
  <>
    ...
    {f.role === 'SUPERVISOR' && (
      <div style={S.profSec}>
        <div style={S.profTitle}>⬡ Supervisor Profile</div>
        <div>
          <label style={S.label}>Monthly Salary (₹) <span style={{ color: '#dc2626' }}>*</span></label>
          <input className="pe-input" style={S.input} type="number" min="0" step="0.01"
            value={f.monthly_salary}
            onChange={e => setF({ ...f, monthly_salary: e.target.value })}
            placeholder="e.g. 50000" />
          <div style={{ fontSize: 12, color: '#71717a', marginTop: 6 }}>
            ⚡ Used for auto-generating monthly payroll on the 5th.
            Run: <code style={{ background: '#111', padding: '1px 6px', borderRadius: 3, color: '#a1a1aa' }}>python manage.py generate_supervisor_payroll</code>
          </div>
        </div>
      </div>
    )}
    ...
    {isPrimaryAdmin ? (
      <div style={{ ...S.profSec, borderLeft: '2px solid #dc2626' }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: '#52525b', marginTop: 6 }}>⚡ Saving this will update the company name for every user in the system.</div>
        </div>
      </div>
    ) : (
      ...
      <div style={S.profSec}>
        ...
        <div>
          ...
          <input className="pe-input" style={S.input} type="number" min="0" step="0.01"
            ...
            placeholder="e.g. 50000" />
          <div style={{ fontSize: 12, color: '#71717a', marginTop: 6 }}>
            ⚡ After saving, run: <code style={{ background: '#111', padding: '1px 6px', borderRadius: 3, color: '#a1a1aa' }}>python manage.py generate_supervisor_payroll</code>
          </div>
        </div>
      </div>
    )}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24, paddingBottom: 20, borderBottom: '1px solid #1a1a1a' }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
              ...
              <span style={{ fontSize: 9, color: roleColor, background: `${roleColor}15`, padding: '2px 8px', borderRadius: 8, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase' }}>{target.role}</span>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <button type="button" onClick={() => setShowPw(!showPw)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#52525b', cursor: 'pointer', padding: 0 }}>
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <button
          ...
          style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '10px 16px', background: revealAll ? 'rgba(220,38,38,0.1)' : '#0d0d0d', border: `1px solid ${revealAll ? '#dc2626' : '#1e1e1e'}`, borderRadius: 6, color: revealAll ? '#fca5a5' : '#a1a1aa', fontFamily: "'Barlow Condensed',sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', cursor: 'pointer', transition: 'all 0.15s' }}>
      ...
      <div style={{ border: '1px solid #1a1a1a', borderRadius: 8, overflow: 'hidden' }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: '#3f3f46', fontFamily: "'Barlow Condensed',sans-serif", letterSpacing: 4, textTransform: 'uppercase', fontSize: 13 }}>No users found</div>
        ) : filtered.map((u, i) => {
          ...
          return (
            <div key={u.id} className="pe-row" style={{ display: 'grid', gridTemplateColumns: '40px 1.8fr 1fr 1.8fr 60px', alignItems: 'center', padding: '13px 16px', background: i % 2 === 0 ? '#0d0d0d' : '#0a0a0a', borderBottom: i < filtered.length - 1 ? '1px solid #111' : 'none', gap: 8 }}>
              ...
              <span style={{ fontSize: 10, color: roleColor, background: `${roleColor}15`, padding: '3px 10px', borderRadius: 8, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', width: 'fit-content' }}>{u.role}</span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {hasPlain ? (
                  <>
                    ...
                    <button onClick={() => toggleReveal(u.id)} style={{ background: '#141414', border: '1px solid #1e1e1e', color: '#52525b', width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', borderRadius: 4, flexShrink: 0 }}>
                      {visible ? <EyeOff size={12} /> : <Eye size={12} />}
                    </button>
                  </>
                ) : (
                  <span style={{ fontSize: 11, color: '#3f3f46', fontStyle: 'italic' }}>— not recorded —</span>
                )}
              </div>
              ...
              <button
                ...
                style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '6px 10px', background: '#141414', color: '#a1a1aa', border: '1px solid #1e1e1e', borderRadius: 5, cursor: 'pointer', transition: 'all 0.15s' }}>
                <KeyRound size={12} />
              </button>
            </div>
          );
      ...
      <div style={{ marginTop: 12, fontSize: 11, color: '#3f3f46' }}>
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (form.role === 'SUPERVISOR')
        payload.monthly_salary = parseFloat(form.monthly_salary) || 0;
  ...
  const openEdit = async (user: User) => {
    ...
    try {
      if (user.role === 'SUPERVISOR') {
        base.monthly_salary = String((user as any).monthly_salary || '0');
  ...
  const handleEditSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editForm.role === 'SUPERVISOR') {
        payload.monthly_salary = parseFloat(editForm.monthly_salary) || 0;
      }
                      ...
                      <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: '#111', borderRadius: 6, marginBottom: 6, border: `1px solid ${isPrimary ? 'rgba(220,38,38,0.25)' : '#1a1a1a'}` }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            ...
                            {isPrimary && <span style={{ fontSize: 8, color: '#dc2626', background: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.25)', padding: '1px 6px', borderRadius: 8, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase' }}>Primary</span>}
                          </div>
