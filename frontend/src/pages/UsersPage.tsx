...
// ── CreateFormFields — OUTSIDE to prevent remount ──────────────
...
const CreateFormFields = ({ f, setF, contractors, supervisors }: CreateFieldsProps) => (
  <>
    ...
    {/* Company name: read-only info notice for non-HR */}
    {f.role !== 'HR' && (
        ...
        <div>
          ...
          <select className="pe-input" style={S.input} value={f.contractor_id} onChange={e => setF({ ...f, contractor_id: e.target.value })}>
            ...
            {contractors.map((c: any) => (
              <option key={c.id} value={c.id}>{c.user_detail?.first_name} {c.user_detail?.last_name || c.user_detail?.username}{c.company_name ? ` — ${c.company_name}` : ''}</option>
            ))}
          </select>
        </div>
      </div>
...
// ── EditFormFields — OUTSIDE to prevent remount ────────────────
...
const EditFormFields = ({ f, setF, isPrimaryAdmin, contractors, supervisors, companyName, onCompanyChange }: EditFieldsProps) => (
  <>
    ...
    {/* Company name — ONLY primary admin can edit */}
    {isPrimaryAdmin ? (
    ...
    {f.role === 'LABOURER' && (
      <div style={S.profSec}>
        ...
        <div>
          ...
          <select className="pe-input" style={S.input} value={f.contractor_id} onChange={e => setF({ ...f, contractor_id: e.target.value })}>
            <option value="">— No contractor yet —</option>
            {contractors.map((c: any) => <option key={c.id} value={c.id}>{c.user_detail?.first_name} {c.user_detail?.last_name || c.user_detail?.username}</option>)}
          </select>
        </div>
      </div>
    )}
...
export default function UsersPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Find primary admin = first HR user by id (lowest id)
  ...
  const handleEditSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Only primary admin can update company name (propagates to all)
    ...
    <>
      ...
      <div style={{ animation:'pageIn 0.4s cubic-bezier(0.16,1,0.3,1)', color:'white' }}>
        ...
        <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:28, paddingBottom:20, borderBottom:'1px solid #161616', position:'relative' }}>
          ...
          <button style={S.btn} onClick={() => setShowForm(true)}><Plus size={14}/> New User</button>
        </div>
          ...
          <div style={S.overlay}>
            <div style={S.modal}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:24, paddingBottom:20, borderBottom:'1px solid #1a1a1a' }}>
                ...
                <button onClick={() => { setShowForm(false); setForm(defaultForm); }} style={{ background:'#1a1a1a', border:'1px solid #222', color:'#a1a1aa', width:32, height:32, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', borderRadius:6 }}>
                  <X size={16}/>
                </button>
              </div>
